...
import type {
  KkphimDetailPayloadRaw,
  KkphimHomePayloadRaw,
  KkphimListPayloadRaw,
  KkphimMovieRaw,
  KkphimServerEpisodeRaw,
  KkphimServerRaw,
} from "./api";
import { KKPHIM_CATEGORIES } from "./constants";

function absoluteImageUrl(path?: string, imageBaseUrl?: string): string {
...
function normalizeMovie(item: KkphimMovieRaw, imageBaseUrl?: string): CatalogItem {
  return {
    sourceId: "kkphim",
...
function buildPlayEntries(serverData: KkphimServerEpisodeRaw[] = []): ProviderEntry[] {
  return serverData
...
function normalizeServers(episodes: KkphimServerRaw[] = []): ProviderServer[] {
  return episodes
    .map((server, index) => {
...
function pickActiveServerIndex(servers: ProviderServer[]): number {
...
function paginationFrom(
  ...
  fallback: { currentPage?: number; totalPages?: number; totalItems?: number } = {},
) {
  return {
...
export function mapHomePayload(payload: KkphimHomePayloadRaw): CatalogPayload {
  const items: KkphimMovieRaw[] = Array.isArray(payload?.items)
    ? payload.items
    : [];
...
export function mapCategoryPayload(
  slug: string,
  payload: KkphimListPayloadRaw,
): CatalogPayload {
...
export function mapSearchPayload(
  keyword: string,
  payload: KkphimListPayloadRaw,
): CatalogPayload {
  const data = payload && payload.data ? payload.data : {};
...
export function mapDetailPayload(
  slug: string,
  payload: KkphimDetailPayloadRaw,
): ProviderDetail {
