...
import type {
  OphimCategoryPayloadRaw,
  OphimHomePayloadRaw,
  OphimDetailPayloadRaw,
  OphimMovieRaw,
  OphimServerEpisodeRaw,
  OphimServerRaw,
} from "./api";
import { OPHIM_CATEGORIES } from "./constants";

function normalizeImageBase(imageBaseUrl?: string): string {
...
function normalizeMovie(item: OphimMovieRaw, imageBaseUrl?: string): CatalogItem {
  return {
    sourceId: "ophim",
...
function buildPlayEntries(serverData: OphimServerEpisodeRaw[] = []): ProviderEntry[] {
  return serverData
...
function normalizeServers(episodes: OphimServerRaw[] = []): ProviderServer[] {
  return episodes
    .map((server, index) => {
...
export function mapHomePayload(payload: OphimHomePayloadRaw): CatalogPayload {
...
export function mapCategoryPayload(
  slug: string,
  payload: OphimCategoryPayloadRaw,
): CatalogPayload {
  const data = payload && payload.data ? payload.data : {};
